...
export const MULTI_HEAD_LATENT_ATTENTION_MODULE_TITLE =
  "Multi-Head Latent Attention" as const;

/** Stable markers for multi-head-latent-attention module page convergence. */
export const MULTI_HEAD_LATENT_ATTENTION_REQUIRED_MARKERS = [
  MULTI_HEAD_LATENT_ATTENTION_MODULE_TITLE,
  'data-registry-id="module.multi-head-latent-attention"',
  "At a glance",
...
export const MULTI_HEAD_LATENT_ATTENTION_MATH_DEFINITION_MARKERS = [
  'data-attention-schema-variable-definitions="true"',
  'data-math-schema="mha"',
  'data-math-schema="gqa"',
  ...MODULE_ATTENTION_MHA_MATH_VARIABLE_DEFINITION_IDS.map(
...
export const MULTI_HEAD_LATENT_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS =
  MODULE_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS;

export const MULTI_HEAD_LATENT_ATTENTION_GRAPH_THEME_MARKERS = [
  "--xy-node-color",
  "--xy-node-background-color",
  'data-manual-visibility-evidence="registry-graph-flow-node-contrast"',
] as const;
...
export function buildMultiHeadLatentAttentionMathComparisonStub(): string {
...
export function buildMultiHeadLatentAttentionStubBody(): string {
  ...
  const staticMarkers = MULTI_HEAD_LATENT_ATTENTION_REQUIRED_MARKERS.filter(
    (marker) =>
      ...
      marker !== 'data-attention-variant-active="mla"' &&
...
export const MULTI_HEAD_LATENT_ATTENTION_FORBIDDEN_MARKERS = [
  "Variants And Nearby Modules",
  'data-testid="derived-related-docs"',
  'aria-label="Module metadata"',
] as const;

export const MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS = {
  duplicateBodyTitle:
    "duplicate Multi-Head Latent Attention h1 in module body or shell",
...
export function assertMultiHeadLatentAttentionTitleConvergence(
  html: string,
): string | null {
  if (
    countH1BlocksContaining(html, MULTI_HEAD_LATENT_ATTENTION_MODULE_TITLE) > 1
  ) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.duplicateBodyTitle;
  }

  if (
    new RegExp(
      `<h1\\b[^>]*>\\s*${escapeRegExp(MULTI_HEAD_LATENT_ATTENTION_MODULE_TITLE)}\\s*</h1>`,
      "i",
    ).test(html) &&
    html.includes('data-registry-id="module.multi-head-latent-attention"')
  ) {
...
export function assertMultiHeadLatentAttentionChromeConvergence(
  html: string,
): string | null {
  if (html.includes('aria-label="Module metadata"')) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.moduleMetadataCard;
  }
  ...
  if (tagPillCount === 0) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.missingTagPillList;
  }
  if (tagPillCount > 1) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.duplicateTagPillList;
  }

  return assertMultiHeadLatentAttentionTitleConvergence(html);
}

export function assertMultiHeadLatentAttentionSingleGraphConvergence(
  html: string,
): string | null {
  ...
  if (graphCount === 0) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.missingReactFlowGraph;
  }
  if (graphCount > 1) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.duplicateReactFlowGraph;
  }

  return null;
}

export function assertMultiHeadLatentAttentionGraphThemeConvergence(
  html: string,
): string | null {
  const missing = requireSubstrings(
    html,
    MULTI_HEAD_LATENT_ATTENTION_GRAPH_THEME_MARKERS,
  );
  if (missing) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.missingThemedNodeColors;
  }

  return null;
}

export function assertMultiHeadLatentAttentionMathDefinitionsConvergence(
  html: string,
): string | null {
  ...
  if (missing) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.missingMathDefinitions;
  }

  const forbidden = forbidSubstrings(
    mathRegion,
    MULTI_HEAD_LATENT_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS,
  );
  if (forbidden) {
    return MULTI_HEAD_LATENT_ATTENTION_CONVERGENCE_REASONS.forbiddenMathDefinitionTerms;
  }
...
/**
 * Returns the first MLA module page content failure reason, or null when HTML
 * includes converged markers and excludes placeholder-only stubs.
 */
export function assertMultiHeadLatentAttentionModuleConvergence(
  html: string,
): string | null {
  const missing = requireSubstrings(
    html,
    MULTI_HEAD_LATENT_ATTENTION_REQUIRED_MARKERS,
  );
  if (missing) {
    return missing;
  }
  ...
  const forbidden = forbidSubstrings(
    html,
    MULTI_HEAD_LATENT_ATTENTION_FORBIDDEN_MARKERS,
  );
  if (forbidden) {
    return forbidden;
  }

  const chrome = assertMultiHeadLatentAttentionChromeConvergence(html);
  if (chrome) {
    return chrome;
  }
  ...
  const theme = assertMultiHeadLatentAttentionGraphThemeConvergence(html);
  if (theme) {
    return theme;
  }
