...
export const SPARSE_ATTENTION_MODULE_TITLE = "Sparse Attention" as const;

/** Stable markers for sparse-attention module page convergence. */
export const SPARSE_ATTENTION_REQUIRED_MARKERS = [
  SPARSE_ATTENTION_MODULE_TITLE,
  'data-registry-id="module.sparse-attention"',
  "At a glance",
  ...
  'data-table-id="table.sparse-attention-comparison"',
...
export const SPARSE_ATTENTION_MATH_DEFINITION_MARKERS = [
  'data-attention-schema-variable-definitions="true"',
  'data-math-schema="mha"',
  'data-math-schema="gqa"',
  ...MODULE_ATTENTION_MHA_MATH_VARIABLE_DEFINITION_IDS.map(
...
export const SPARSE_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS =
  MODULE_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS;

export const SPARSE_ATTENTION_GRAPH_THEME_MARKERS = [
  "--xy-node-color",
  "--xy-node-background-color",
  'data-manual-visibility-evidence="registry-graph-flow-node-contrast"',
] as const;
...
export function buildSparseAttentionMathComparisonStub(): string {
...
export function buildSparseAttentionStubBody(): string {
  ...
  const staticMarkers = SPARSE_ATTENTION_REQUIRED_MARKERS.filter(
    (marker) =>
...
export const SPARSE_ATTENTION_FORBIDDEN_MARKERS = [
  "Variants And Nearby Modules",
  'data-testid="derived-related-docs"',
  'aria-label="Module metadata"',
] as const;

export const SPARSE_ATTENTION_CONVERGENCE_REASONS = {
  duplicateBodyTitle: "duplicate Sparse Attention h1 in module body or shell",
...
export function assertSparseAttentionTitleConvergence(
  html: string,
): string | null {
  if (countH1BlocksContaining(html, SPARSE_ATTENTION_MODULE_TITLE) > 1) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.duplicateBodyTitle;
  }

  if (
    new RegExp(
      `<h1\\b[^>]*>\\s*${escapeRegExp(SPARSE_ATTENTION_MODULE_TITLE)}\\s*</h1>`,
      "i",
    ).test(html) &&
    html.includes('data-registry-id="module.sparse-attention"')
  ) {
...
export function assertSparseAttentionChromeConvergence(
  html: string,
): string | null {
  if (html.includes('aria-label="Module metadata"')) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.moduleMetadataCard;
  }
  ...
  if (tagPillCount === 0) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.missingTagPillList;
  }
  if (tagPillCount > 1) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.duplicateTagPillList;
  }

  return assertSparseAttentionTitleConvergence(html);
}

export function assertSparseAttentionSingleGraphConvergence(
  html: string,
): string | null {
  ...
  if (graphCount === 0) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.missingReactFlowGraph;
  }
  if (graphCount > 1) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.duplicateReactFlowGraph;
  }

  return null;
}

export function assertSparseAttentionGraphThemeConvergence(
  html: string,
): string | null {
  const missing = requireSubstrings(html, SPARSE_ATTENTION_GRAPH_THEME_MARKERS);
  if (missing) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.missingThemedNodeColors;
  }

  return null;
}

export function assertSparseAttentionMathDefinitionsConvergence(
  html: string,
): string | null {
  ...
  if (missing) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.missingMathDefinitions;
  }

  const forbidden = forbidSubstrings(
    mathRegion,
    SPARSE_ATTENTION_MATH_FORBIDDEN_DEFINITION_TERMS,
  );
  if (forbidden) {
    return SPARSE_ATTENTION_CONVERGENCE_REASONS.forbiddenMathDefinitionTerms;
  }
...
/**
 * Returns the first sparse-attention module page content failure reason, or null
 * when HTML includes converged markers and excludes placeholder-only stubs.
 */
export function assertSparseAttentionModuleConvergence(
  html: string,
): string | null {
  const missing = requireSubstrings(html, SPARSE_ATTENTION_REQUIRED_MARKERS);
  if (missing) {
    return missing;
  }

  const forbidden = forbidSubstrings(html, SPARSE_ATTENTION_FORBIDDEN_MARKERS);
  if (forbidden) {
    return forbidden;
  }

  const chrome = assertSparseAttentionChromeConvergence(html);
  if (chrome) {
    return chrome;
  }

  const singleGraph = assertSparseAttentionSingleGraphConvergence(html);
  if (singleGraph) {
    return singleGraph;
  }

  const theme = assertSparseAttentionGraphThemeConvergence(html);
  if (theme) {
    return theme;
  }

  const mathDefinitions = assertSparseAttentionMathDefinitionsConvergence(html);
  if (mathDefinitions) {
