import { extractBatch013CustomerAskRowsFromReport } from "./batch-013-customer-ask-check-inventory";
...
export const PHASE_1_BATCH_013_CONVERGENCE_EVIDENCE_SUMMARY_HEADER =
  "Phase 1 batch-013 convergence evidence summary";

export const BATCH_013_CUSTOMER_ASK_CONVERGENCE_DOMAIN_ID =
  "customer-ask-convergence" as const;

export const BATCH_013_CUSTOMER_ASK_CONVERGENCE_DOMAIN_LABEL =
  "Customer-ask convergence checks";

export const BATCH_013_CUSTOMER_ASK_CONVERGENCE_CHECKLIST_ROW =
  "phase-1-batch-013-customer-ask-convergence";

export type Phase1Batch013ConvergenceRecommendation =
  | "queue-one-narrow-repair-batch"
  | "stop-and-wait-for-phase-advancement";

export type Batch013CustomerAskConvergenceDomainSummary = {
  domainId: typeof BATCH_013_CUSTOMER_ASK_CONVERGENCE_DOMAIN_ID;
  label: string;
  checklistRow: string;
...
export type Phase1Batch013ConvergenceEvidenceSummary = {
  commandPath: VerifierCommandPathEvidence;
  customerAsk: Batch013CustomerAskConvergenceDomainSummary;
  recommendation: Phase1Batch013ConvergenceRecommendation;
  recommendationRationale: string;
};

export type BuildPhase1Batch013ConvergenceEvidenceSummaryInput = {
  verifyOutput: string;
...
function summarizeCustomerAskDomain(
  rows: readonly CustomerAskConvergenceRow[],
): Batch013CustomerAskConvergenceDomainSummary {
  return {
    domainId: BATCH_013_CUSTOMER_ASK_CONVERGENCE_DOMAIN_ID,
    label: BATCH_013_CUSTOMER_ASK_CONVERGENCE_DOMAIN_LABEL,
    checklistRow: BATCH_013_CUSTOMER_ASK_CONVERGENCE_CHECKLIST_ROW,
    status: aggregateStatuses(rows.map((row) => row.status)),
    rows,
  };
}

function resolveBatch013CustomerAskRows(
  verifyOutput: string,
  customerAskRows: readonly CustomerAskConvergenceRow[] | undefined,
): CustomerAskConvergenceRow[] {
  if (customerAskRows !== undefined) {
    return [...customerAskRows];
  }

  return extractBatch013CustomerAskRowsFromReport(
    parseCustomerAskConvergenceReport(verifyOutput),
  );
}

export function derivePhase1Batch013ConvergenceRecommendation(input: {
  commandPath: VerifierCommandPathEvidence;
  customerAskRows: readonly CustomerAskConvergenceRow[];
}): {
  recommendation: Phase1Batch013ConvergenceRecommendation;
  ...
  if (
    ...
    return {
      ...
      rationale: `Batch-013 evidence failed: ${failureParts.join("; ")}. Queue one narrow repair batch before Phase 1 stop-and-wait.`,
    };
  }
  ...
  return {
    recommendation: "stop-and-wait-for-phase-advancement",
    rationale:
      "Verifier command-path passed and all customer-ask rows passed. Stop and wait for customer Phase 1 advancement after batch-013 repairs.",
  };
}

/**
 * Merges verifier command-path evidence with batch-013 reopened-row customer-ask
 * convergence rows into one planner-facing batch-013 convergence summary.
 */
export function buildPhase1Batch013ConvergenceEvidenceSummary(
  input: BuildPhase1Batch013ConvergenceEvidenceSummaryInput,
  ...
  const customerAskRows = resolveBatch013CustomerAskRows(
    input.verifyOutput,
...
export function formatBatch013CustomerAskConvergenceDomainLine(
  summary: Batch013CustomerAskConvergenceDomainSummary,
): string {
  const status = summary.status.toUpperCase();
  return `[${status}] ${summary.domainId} — ${summary.label} — checklistRow=${summary.checklistRow}`;
}

export function formatPhase1Batch013ConvergenceEvidenceSummary(
  summary: Phase1Batch013ConvergenceEvidenceSummary,
): string {
  const lines = [PHASE_1_BATCH_013_CONVERGENCE_EVIDENCE_SUMMARY_HEADER];
  lines.push(formatVerifierCommandPathEvidenceLine(summary.commandPath));
  lines.push(
    formatBatch013CustomerAskConvergenceDomainLine(summary.customerAsk),
  );
...
export type PrintPhase1Batch013ConvergenceEvidenceSummaryOptions = {
  writeLine?: (line: string) => void;
};

export function printPhase1Batch013ConvergenceEvidenceSummary(
  summary: Phase1Batch013ConvergenceEvidenceSummary,
  options: PrintPhase1Batch013ConvergenceEvidenceSummaryOptions = {},
): void {
  const writeLine = options.writeLine ?? ((line: string) => console.log(line));
  for (const line of formatPhase1Batch013ConvergenceEvidenceSummary(
    summary,
  ).split("\n")) {
    writeLine(line);
  }
...
/**
 * Batch-013 convergence exit semantics: fail when verifier command-path or any
 * customer-ask row fails. Uncertain evidence is non-blocking (exit 0).
 */
export function getPhase1Batch013ConvergenceExitCode(
  summary: Phase1Batch013ConvergenceEvidenceSummary,
): 0 | 1 {
...
export const PHASE_1_BATCH_013_CONVERGENCE_WORKFLOW_STEPS = [
  "make build",
  "make internal-verify-phase-1-ux",
] as const;

export const PHASE_1_BATCH_013_CONVERGENCE_PREREQUISITES = [
