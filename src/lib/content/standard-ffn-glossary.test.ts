...
describe("FFN pages from phase 3 standard FFN glossary page (US-001)", () => {
  test("registry record is published with dense FFN aliases and curated related ids", () => {
  ...
  test("curated related links broad FFN, mixture of experts, and activation", () => {
  ...
  test("messages explain the dense expand activate project path after attention", () => {
  ...
  test("page renders baseline FFN explanation and curated related links", async () => {
    ...
    expectHtmlToContainProse(html, "plain dense feed-forward block");
  ...
  test("search index records Standard FFN with glossary kind and exact aliases", async () => {
    ...
    expect(document?.aliases).toEqual(
      expect.arrayContaining(["standard FFN", "dense FFN"]),
    );
  });
});
