...
describe("Phase 1 published-resource discovery contract", () => {
  test(
    "grouped-query-attention resolves as an attention resource from registry and frontmatter tags",
    async () => {
  ...
  test(
    "tag landing and canonical loader agree on attention-tagged published resources",
    async () => {
      const indexes = await loadRegistry();
  ...
  test(
    "search documents derive attention tags from the same published-resource rule",
    async () => {
      const indexes = await loadRegistry();
      const canonicalPages = await loadPublishedResourcesForTag(
        PHASE_1_ATTENTION_TAG_SLUG,
      ...
      const gqaDocument = documents.find(
        (document) =>
          document.url === PHASE_1_GROUPED_QUERY_ATTENTION_MODULE_URL,
      );
  ...
  test(
    "attention module URLs are derived from published resources and stay module-scoped",
    async () => {
      const indexes = await loadRegistry();
