...
describe("Phase 3 feed-forward network glossary page (US-002)", () => {
  ...
  test("curated related links transformer architecture and planned mixture of experts", () => {
    ...
    expect(moe?.isPlanned).toBe(true);
    expect(moe?.href).toBeUndefined();
    expect(moe?.reasonLabel).toBe(PLANNED_RELATED_REASON_LABEL);
  });
