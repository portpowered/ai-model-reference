...
describe("sliding-window-attention canonical page template convergence", () => {
  test("published page.mdx follows hardened module template structure", () => {
    const template = readFileSync(
      join(SLIDING_WINDOW_ATTENTION_PAGE_DIR, "page.mdx"),
      "utf8",
    );
    ...
    expect(template).toContain(
      '<ModuleGraph registryId="module.sliding-window-attention" assetId="computeFlow" />',
    );
  ...
  test("published messages use folded summary and symbol math keys", () => {
    const messages = JSON.parse(
      readFileSync(
        join(SLIDING_WINDOW_ATTENTION_PAGE_DIR, "messages/en.json"),
        "utf8",
      ),
    ) as {
    ...
    expect(messages.math?.gqaSchema?.variableDefinitions?.g?.term).toBe("W");
  });

  test("published assets configure sliding-window attention-variant graph and comparison table", () => {
    const assets = JSON.parse(
      readFileSync(
        join(SLIDING_WINDOW_ATTENTION_PAGE_DIR, "assets.json"),
        "utf8",
      ),
