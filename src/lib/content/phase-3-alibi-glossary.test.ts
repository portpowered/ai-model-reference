...
describe("Phase 3 ALiBi glossary page (US-010)", () => {
  ...
  test("curated related links positional encodings and RoPE with navigable hrefs and planned context window", () => {
    ...
    expect(contextWindow?.isPlanned).toBe(true);
    expect(contextWindow?.href).toBeUndefined();
    expect(contextWindow?.reasonLabel).toBe(PLANNED_RELATED_REASON_LABEL);
  });
