...
describe("multi-head-latent-attention canonical page template convergence", () => {
  test("published page.mdx follows hardened module template structure", () => {
    const template = readFileSync(
      join(MULTI_HEAD_LATENT_ATTENTION_PAGE_DIR, "page.mdx"),
      "utf8",
    );
    ...
    expect(template).toContain(
      '<ModuleGraph registryId="module.multi-head-latent-attention" assetId="computeFlow" />',
    );
  ...
  test("published messages use folded summary and symbol math keys", () => {
    const messages = JSON.parse(
      readFileSync(
        join(MULTI_HEAD_LATENT_ATTENTION_PAGE_DIR, "messages/en.json"),
        "utf8",
      ),
    ) as {
  ...
  test("published assets configure MLA attention-variant graph and comparison table", () => {
    const assets = JSON.parse(
      readFileSync(
        join(MULTI_HEAD_LATENT_ATTENTION_PAGE_DIR, "assets.json"),
        "utf8",
      ),
