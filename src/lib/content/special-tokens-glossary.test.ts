...
const SPECIAL_TOKENS_TIMEOUT_MS = 15_000;
...
describe("special tokens glossary page (special-tokens-page-002)", () => {
  ...
  test(
    "page renders the canonical glossary route with practical links to tokenization and prompting surfaces",
    async () => {
    ...
    { timeout: SPECIAL_TOKENS_TIMEOUT_MS },
  );

  test(
    "registry record, shipped route, and default English messages resolve the same canonical search document",
    async () => {
  ...
  test.each([
    ...
    "%s query routes discovery into the canonical special-tokens page",
    async (query) => {
  ...
  test(
    "token glossary renders a navigable related-doc link to special tokens",
    async () => {
  ...
  test(
    "neighboring tokenizer, prompt-adjacent, and vocabulary pages render navigable links back to special tokens",
    async () => {
      ...
      expect(conditioningHtml).toContain(
