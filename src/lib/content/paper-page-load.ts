...
export async function loadPaperPageFromDisk(
  slug: string,
  locale = "en",
  papersDocsRoot = PAPERS_DOCS_ROOT,
): Promise<LoadedPaperPage> {
  const pageDir = join(papersDocsRoot, slug);
  ...
  registerPageGraphRecords(join(papersDocsRoot, "..", ".."), assets);
