...
describe("local-attention page messages", () => {
  test("includes required localized fields for the module template", () => {
    ...
    expect(messages.math?.gqaSchema?.variableDefinitions?.g?.term).toBe("r");
    expect(messages.math?.gqaSchema?.variableDefinitions?.gi?.term).toBe("L_r");
    expect(
...
describe("loadModulePage local-attention", () => {
  test("compiles MDX with local namespaces and local-attention-specific discovery links", async () => {
    ...
    expect(html).toContain("query attends only to a nearby neighborhood");
    expect(html).toContain("specific fixed-width");
    expect(html).toContain("instead of only a neighborhood");
    ...
    expect(html).toContain("Broad umbrella for nearby-only neighborhoods");
    expect(html).toContain(
      'data-graph-id="graph.local-attention-neighborhood-pattern"',
    );
    expect(html).toContain('data-graph-node-id="local-time-current-query"');
    expect(html).toContain('data-graph-node-id="local-time-kv-i-plus-2"');
  });
});
  ...
  test("resolves graph and table assets with message-backed alt text", () => {
    ...
    if (assets.computeFlow.type === "attention-variant-graph") {
      expect(
        ...
        "graph.local-attention-neighborhood-pattern",
      ]);
    }
