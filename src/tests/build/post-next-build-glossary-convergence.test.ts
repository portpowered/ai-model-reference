...
/**
 * Built `.next/server` glossary HTML convergence probes run after
 * `next-build-tracing-warning.test.ts` so they read the in-suite production
 * build instead of stale `.next` artifacts left from an earlier local build.
 */
