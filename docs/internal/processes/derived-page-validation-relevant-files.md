...
- `src/lib/content/gemma-slice-verification.test.tsx` — consolidated review-facing proof for the Gemma model-family slice: route/registry alignment, Google/DeepMind primary-source citation resolution and rendered references, architecture graph surface, representative alias search, derived-related discovery paths, and optional served-page browser verification; routine bundle invariants stay on story-scoped source-backing, model-page, discovery, architecture-tradeoffs, and architecture-graph tests plus `validateDerivedPublishedPageBundles`.
