...
- `src/lib/content/gpu-system-slice-verification.test.ts` — consolidated review-facing proof for the GPU system slice: route/registry alignment, graph/citation resolution, alias search, hardware-distributed tag landing, adjacent serving/concept link resolution, and rendered system shell surface; routine bundle invariants stay on `validateDerivedPublishedPageBundles`.
