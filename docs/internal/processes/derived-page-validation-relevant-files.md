...
- `src/lib/content/time-to-first-token-slice-verification.test.tsx` — consolidated review-facing proof for the TTFT glossary slice; serving metric pages with only derived bundle invariants plus a metric comparison table can rely on `make validate-data` and `bun run audit:canonical-page-surface` on page-owned paths without adding another slice file.
...
- Publishing `concept.inter-token-latency` with alias `latency` enables prose auto-links on other pages; when a downstream HTML substring test breaks, prefer href or fragment assertions that survive auto-linking (see `memory-system-page.test.ts`) over adding a new slice verification file.
