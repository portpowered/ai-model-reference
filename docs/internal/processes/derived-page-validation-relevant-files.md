...
- `src/lib/content/softmax-concept-slice-verification.test.ts` — consolidated review-facing proof for the softmax concept slice: route/registry alignment, citation resolution, alias search with glossary co-discovery, logit prerequisite related-doc handoff, sampling-neighbor curated-related hrefs and reasons, token-to-probability-chain tag landing, and rendered concept surface; routine bundle invariants stay on `validateDerivedPublishedPageBundles`.
