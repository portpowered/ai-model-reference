...
- `src/lib/content/synthetic-data-slice-verification.test.tsx` — consolidated review-facing proof for the synthetic data concept slice: route/registry alignment, citation resolution, alias search, curated-related training-workflow hrefs, and rendered concept surface; routine bundle invariants stay on `validateDerivedPublishedPageBundles`.
